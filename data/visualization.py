...
def compare_yield_bar(selected_crop, first_opt, second_opt, state, unit, filter, legend_flag, c_data, custom_crop_value):
    ...
    df = df.sort_values(by=col2, key=lambda x: x.map(
...
def compare_yield_box(selected_crop, first_opt, second_opt, state, unit, filter, legend_flag, c_data, custom_crop_value):
    if selected_crop != "Custom":
        df = (datasets[selected_crop]).copy(deep=True)
        df = df[df.STATE == state]
        c_unit = selected_crop
    ...
    fig = px.box(
        ...
        labels={'MOIST': 'Moist', 'YIELD': f'Yield ({unit_str})',
                'WATER_REGIME': 'Water Regime', 'NAME': 'Genotype', 'DAYS': 'Days', 'YEAR': 'Year'},
    )
...
def compare_county_yield_bar_graph(selected_crop, first_opt, second_opt, state, unit, filter, c_data, custom_crop_value):
    if selected_crop != "Custom":
        df = (datasets[selected_crop]).copy(deep=True)
        df = df[df.STATE == state]
        c_unit = selected_crop
    ...
    df = df[df[col2].isin(second_opt)].groupby(
        [col2, 'WATER_REGIME', 'COUNTY_CITY'])['YIELD'].mean().reset_index()

    df = df.sort_values(by=col2, key=lambda x: x.map(
        dict(zip(second_opt, range(len(second_opt))))))
    fig = px.bar(df, x='COUNTY_CITY', y='YIELD',
                 color_discrete_map=color_map,
                 facet_col=col2,
                 color='WATER_REGIME', barmode='group',
                 labels={'NAME': 'Name', 'YIELD': f'Yield ({unit_str})',
                         'WATER_REGIME': 'Water Regime', 'YEAR': 'Year', 'COUNTY_CITY': 'County/City'})
    ...
    for i, county in enumerate(df['COUNTY_CITY'].unique()):
        ...
        fig.add_trace(px.scatter(curr, x='County/City',
                      y='Average Yield', color_discrete_sequence=['#6A41D5']).data[0], row='all', col='all')
        fig.add_shape(
            type='line',
            x0=i - 0.3,
            x1=i + 0.3,
